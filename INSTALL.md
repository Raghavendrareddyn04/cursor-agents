...
| [`deploy/`](deploy/) — Minikube base, Helm values, Grafana provisioning, deploy scripts | `deploy/.secrets/` (if used), live certs |
...
| `docker-compose*.yml` in the **app repo** (generated by JHipster — dev/CI only) | `.env`, `*.pem`, `*.key`, `letsencrypt/` |
| Docs (`README.md`, `INSTALL.md`, [`DEPLOY-ASSETS.md`](DEPLOY-ASSETS.md), …) | `graphify-out/` (regenerated) |
| [`.env.example`](.env.example) (template only) | Real `.env` with passwords |

### Two runtime models (do not mix them up)
